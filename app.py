...
# --- DISEÑO UI MOBILE (ESTILO 888 VERTICAL) ---
st.markdown("""
    <style>
    .main { background-color: #06080a; color: #e0e0e0; }
    .poker-arena {
        position: relative; width: 100%; max-width: 320px; height: 460px;
...
# Coordenadas ajustadas para móvil vertical
...
# Lógica del Board (Cartas comunitarias)
v_count = 0
...
with col1:
    if st.button("🚀 RAISE"): advance(g['pot'] * 2); st.rerun()
with col2:
    if st.button("👀 CALL/CHECK"): advance(0); st.rerun()
with col3:
